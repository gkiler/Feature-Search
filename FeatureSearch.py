...
class Classifier:
    ...
    def Euclidean_Distance(self,one,two):
        ...
        # print(list(two[1:]))
        # print(list(one[1][1:]))
        for (x,y) in zip(list(one[1][1:]),list(two[1:])):
            sum += (x-y)**2 
        return (sum**.5)
...
class Node:
    ...
    def remove(self,feature):
        placeholder = [i for i in self.features if i.id != feature.id]
...
def ForwardSelection(features,df):
    ...
    for i in range(1,features_len+1):
        ...
        for feature in remaining_features.features:
            ...
            v = Validator(test_features.features,Classifier(),df)
